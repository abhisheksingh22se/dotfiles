...
-- ═════════════════════════════════════════════
-- DO NOT LET `ambxst install hyprland` APPEND HERE
-- ═════════════════════════════════════════════
-- That command appends this line to the bottom of whatever Hyprland config it finds:
--
--     loadfile(os.getenv("HOME") .. "/.local/share/ambxst/hyprland.lua")()
--
-- Delete it if it appears below. The Ambxst section further up (search
-- `ambxst_session`) already does the same job, and the appended version breaks the
-- machine in two ways this file exists to avoid:
--
--   1. It is unguarded. `loadfile` returns nil for a missing file rather than
--      raising, so calling its result is "attempt to call a nil value" and Hyprland
--      refuses the whole config. ~/.local/share/ambxst/hyprland.lua is *generated by
--      Ambxst at runtime*, so on any boot before Ambxst has run it is absent — and
--      Ambxst cannot run, because Hyprland will not start. That is a deadlock, and it
--      takes the bare `hypr` session down with it.
--   2. It runs in both sessions, loading Ambxst's keybinds into a session where
--      nothing is listening for them.
--
-- ~/.config/hypr is a stow symlink into this repo, so the append lands in the working
-- tree and shows up as a local modification. `git checkout -- hypr/` reverts it.
